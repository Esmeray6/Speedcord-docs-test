...
class OpcodeDispatcher:
    """
    Receives events identified by their opcode, and handles them by
    running them through the event loop.
    ...
    def register(self, opcode, func):
        """
        Register a handler for a specific opcode. This handler will be
        called whenever a matching opcode is dispatched.
...
class EventDispatcher:
    """
    Receives events identified by their name and handles them by running
    them through the event loop.
    ...
    def register(self, event_name, func):
        """
        Register a handler for a specific event. This handler will be called
        whenever an event matching the registered event_name is dispatched.
