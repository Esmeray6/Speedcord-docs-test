...
class Route:
    ...
    For a list of routes and their parameters, refer to
...
class LockManager:
    """
    Used by HttpClient to handle rate limits. Locked when a Bucket's
    rate limit has been hit, which prevents additional requests from
    being executed.
...
class HttpClient:
    """
    An HTTP client that handles Discord ratelimits.

    Parameters
    ----------
    token: str
        A Discord bot token. To create a bot -
    ...
    **loop: AbstractEventLoop
    ...
    async def create_ws(self, url, *, compression) -> ClientWebSocketResponse:
        ...
        compression: int
            Whether to enable compression. Refer to
            https://discord.com/developers/docs/topics/gateway for more
            information.
        """
        options = {
            "max_msg_size": 0,
    ...
    async def request(self, route: Route, **kwargs):
        """
        Sends a request to the Discord API. Handles rate limits by utilizing
        LockManager and the Discord API Bucket system -
        https://discord.com/developers/docs/topics/gateway#encoding-and
        -compression.

        When the client wants to send a new request, this method attempts to
        acquire a ratelimit lock. When it eventually does, it sends a
        request and checks to see if the ratelimit has been exceeded. If so,
        that Bucket's LockManager is locked so other requests cannot acquire
        a lock.
        The Discord Bucket system returns a `delta` value which
        specifies how long it will take before another request can be sent
        and the LockManager for that Bucket can be unlocked.
